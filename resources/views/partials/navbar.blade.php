        ...
        <div class="flex flex-wrap lg:grid lg:grid-cols-3 items-center mx-auto max-w-screen-xl">
            ...
            <div class="hidden justify-center items-center w-full lg:flex lg:w-auto">
                <ul class="flex flex-row space-x-8 font-medium">
                    <li><a href="{{ route('home') }}" class="block py-2 pr-4 pl-3 text-lg font-bold rounded lg:p-0 {{ request()->is('/') ? 'text-white' : 'text-white/70 hover:text-white' }}">Home</a></li>
                    <li><a href="{{ route('about') }}" class="block py-2 pr-4 pl-3 text-lg font-bold rounded lg:p-0 {{ request()->is('about*') ? 'text-white' : 'text-white/70 hover:text-white' }}">About</a></li>
                    <li><a href="{{ route('classes') }}" class="block py-2 pr-4 pl-3 text-lg font-bold rounded lg:p-0 {{ request()->is('classes*') ? 'text-white' : 'text-white/70 hover:text-white' }}">Classes</a></li>
                    <li><a href="{{ route('contact') }}" class="block py-2 pr-4 pl-3 text-lg font-bold rounded lg:p-0 {{ request()->is('contact*') ? 'text-white' : 'text-white/70 hover:text-white' }}">Contact</a></li>
                </ul>
            </div>

            <div class="flex items-center justify-end ml-auto lg:ml-0 sm:gap-4">
                ...
                @auth
                    <div class="relative">
                        ...
                        <div x-show="profileOpen" 
                             ...
                             class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-2 z-50 border text-gray-700">
                            <div class="px-4 py-2 border-b">
                                <p class="text-sm font-bold text-gray-900">{{ auth()->user()->name }}</p>
                                <p class="text-xs text-gray-500 truncate">{{ auth()->user()->email }}</p>
                            </div>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-blue-50">Dashboard</a>
                            <a href="#" class="block px-4 py-2 text-sm hover:bg-blue-50">Settings</a>
                            <form action="" method="POST" class="border-t mt-1">
                                @csrf
            ...
            <div x-show="mobileMenuOpen" 
                ...
                <ul class="flex flex-col font-bold text-white py-4 space-y-4">
                    <li><a href="{{ route('home') }}" class="block py-2 px-3 rounded {{ request()->is('/') ? 'bg-white/20' : 'hover:bg-white/10' }}">Home</a></li>
                    <li><a href="{{ route('about') }}" class="block py-2 px-3 rounded {{ request()->is('about*') ? 'bg-white/20' : 'hover:bg-white/10' }}">About</a></li>
                    <li><a href="{{ route('classes') }}" class="block py-2 px-3 rounded {{ request()->is('classes*') ? 'bg-white/20' : 'hover:bg-white/10' }}">Classes</a></li>
                    <li><a href="{{ route('contact') }}" class="block py-2 px-3 rounded {{ request()->is('contact*') ? 'bg-white/20' : 'hover:bg-white/10' }}">Contact</a></li>
                    @guest
