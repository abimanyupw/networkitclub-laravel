...
@section('content')
    ...
    <div class="mx-auto">
        {{-- PERBAIKAN: Gunakan $category->slug dan method PUT --}}
        <form action="{{ route('managecategory.update', ['managecategory' => $category->slug]) }}" method="POST">
            @csrf
            @method('PUT')
                        ...
                        <div class="p-8 space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="slug" name="slug" value="{{ old('slug', $category->slug) }}" readonly
                                           class="w-full px-4 py-2.5 rounded-xl border @error('slug') border-red-500 @else border-gray-200 dark:border-gray-700 @enderror bg-gray-200 dark:bg-slate-700 dark:text-gray-400 focus:ring-2 focus:ring-blue-500 transition outline-none">
                                    @error('slug') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div> 
                                <div class="col-span-1 md:col-span-2">
                                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Description</label>
                                    <textarea name="description" class="w-full h-30 px-4 py-2.5 rounded-xl border @error('description') border-red-500 @else border-gray-200 dark:border-gray-700 @enderror bg-gray-50 dark:bg-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 transition outline-none">{{ old('description', $category->description) }}</textarea>
