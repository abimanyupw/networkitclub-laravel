...
<div class="min-h-screen text-gray-900 dark:text-white">
    <div class="mb-8">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white italic">Daftar Kategori Materi </h1>
        <nav class="mt-2 flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400">
            ...
            <a href="{{ route('managecategory.index') }}" class="hover:underline font-medium">Manajemen Kategori Materi </a>
            <span class="text-gray-400">/</span>
            <span class="text-gray-500 dark:text-gray-400 font-medium">Daftar Kategori Materi </span>
        </nav>
    </div>
    ...
    <div class="mb-6 flex flex-wrap gap-2">
        <a href="/managecategory/create" class="flex items-center gap-2 rounded bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-700 transition shadow-sm">
    ...
    {{-- Filter & Search --}}
    <form action="/managecategory" method="GET" class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div>
            <label class="mb-1 block text-sm font-medium">Cari Kategori Materi :</label>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm">
                ...
                <tr class="bg-white/90 hover:bg-blue-50/50 dark:bg-slate-900 dark:hover:bg-slate-800/80 transition-colors">
                    <td class="border-b border-r border-gray-300 px-4 py-3 dark:border-gray-800 text-center font-medium">{{ ($categories->currentPage() - 1) * $categories->perPage() + $loop->iteration }}</td>
                    ...
                    <td class="border-b border-gray-300 px-4 py-3 dark:border-gray-800">     
                        <div class="flex justify-center gap-2">
                            <a href="/managecategory/{{ $category->slug }}" class="bg-sky-400 p-1.5 rounded hover:bg-sky-500 transition" title="View">
                            ...
                            <a href="/managecategory/{{ $category->slug }}/edit" class="bg-amber-400 p-1.5 rounded hover:bg-amber-500 transition" title="Edit">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </a>
                            <form action="/managecategory/{{ $category->slug }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                                        class="cursor-pointer bg-rose-500 p-1.5 rounded hover:bg-rose-600 transition shadow-sm" 
                                        title="Hapus"
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus kategori materi ini?')">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
